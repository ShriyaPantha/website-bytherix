    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-6">
        <div className="text-center">
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-cyan-500">Bytherix Team</p>
    ...
    <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <header className="border-b border-black/5">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-10">
          <button type="button" onClick={handleBack} className="group flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-[#3157D5] transition-opacity duration-300 hover:opacity-60">
            <ArrowLeft size={15} className="transition-transform duration-300 group-hover:-translate-x-1 text-[#3157D5]" />
            Back to Team
          </button>
      ...
      {/* PROFILE HERO */}
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="mx-auto max-w-7xl px-5 pb-8 pt-12 sm:px-8 sm:pb-10 sm:pt-16 lg:px-10 lg:pb-12 lg:pt-24">
        <div className="grid items-start gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.05 }} className="relative overflow-hidden rounded-[24px] bg-[#F3F5F8] sm:rounded-[28px]">
            ...
            <img src={member.image} alt={member.name} className="h-[420px] w-full object-cover object-center sm:h-[540px] lg:h-[620px]" />
          </motion.div>
      ...
      {/* BIO / JOURNEY — directly below the social icons, full width, justified */}
