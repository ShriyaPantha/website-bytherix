...
const Hero = ({ docked }: HeroProps) => {
  return (
    <section className="relative isolate z-10 w-full overflow-hidden min-h-[100svh] py-8 sm:py-10 md:min-h-[calc(100svh-4rem)] md:py-12 lg:flex lg:min-h-[640px] lg:h-[calc(100vh-6rem)] lg:items-center lg:py-0">
      ...
      {/* HERO CANVAS */}
      <div className="relative z-20 mx-auto flex w-full max-w-[1400px] flex-col px-4 sm:px-6 md:px-8 lg:px-0">
        {/* MAIN HERO */}
        ...
        <div className="relative z-40 w-full mt-8 sm:mt-10 md:mt-8 lg:mt-8 xl:mt-8 pb-2 sm:pb-4 lg:pb-4">
          <div className="relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/[0.10] bg-black/[0.20] backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.35)] px-3 py-3 sm:px-4 sm:py-4 md:px-5 md:py-4 lg:px-6 lg:py-4 before:pointer-events-none before:absolute before:inset-0 before:bg-[linear-gradient(90deg,transparent,rgba(56,189,248,0.045),transparent)]">
            <div className="grid grid-cols-2 items-stretch sm:grid-cols-4 gap-y-0 divide-x divide-white/[0.07]">
              {STATS.map((stat) => (
                <div key={stat.label} className="group relative min-w-0 flex flex-col justify-center px-3 py-2 sm:px-3 sm:py-1 md:px-4 lg:px-6 transition-colors duration-300">
                  ...
                  <div className="flex min-w-0 items-center gap-2.5 sm:gap-3 md:gap-3.5">
                    ...
                    <div className="min-w-0 flex flex-1 flex-col">
                      <p className="truncate text-lg font-bold leading-none tracking-[-0.025em] text-white sm:text-xl md:text-2xl lg:text-[1.5rem]">
                        {stat.value}
                      </p>

                      <p className="mt-1 truncate text-[8px] font-semibold uppercase tracking-[0.08em] text-white/60 sm:text-[9px] md:text-[10px] lg:text-xs lg:tracking-[0.1em]">
                        {stat.label}
                      </p>

                      <p className="mt-1 hidden text-[10px] leading-relaxed text-white/35 md:block lg:text-[11px]">
                        {stat.desc}
                      </p>
                    </div>
                  </div>
