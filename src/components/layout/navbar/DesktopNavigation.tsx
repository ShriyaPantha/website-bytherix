...
import { Search, Heart, Bell, UserRound, ChevronDown } from "lucide-react";
import { NAV_ITEMS, DROPDOWN_CONTENT, dropdownVariants } from "./navbar.constants";
...
const DesktopNavigation = ({
  ...
  return (
    <>
      <motion.div initial={false} animate={{ opacity: docked ? 1 : 0, x: docked ? 0 : 10 }} transition={{ duration: 0.35, delay: docked ? 0.1 : 0 }} className="relative z-20 ml-auto hidden items-center gap-4 lg:flex xl:gap-5 2xl:gap-6">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <div key={item.label} className="relative py-2" onMouseEnter={() => { setHoveredNavItem(item.label); setActiveDropdown(item.label); }} onMouseLeave={() => setHoveredNavItem(null)}>
                <button type="button" onClick={() => setActiveDropdown(isDropdownOpen ? null : item.label)} className={`group relative flex items-center gap-1.5 whitespace-nowrap font-['Inter'] text-[12px] font-bold uppercase tracking-[0.035em] transition-all duration-200 ${isHovered ? "text-[#00AEEF]" : "text-white/85"}`}>
                  {item.label}
                  <ChevronDown size={12} strokeWidth={2} className={`transition-all duration-200 ${isHovered ? "rotate-180 text-[#00AEEF]" : "text-white/65"}`} />
        ...
        <button type="button" onClick={() => navigateTo("/#contact")} className="whitespace-nowrap rounded-full bg-[#3154C4] px-5 py-2.5 font-['Inter'] text-xs font-bold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#3B5FD0] hover:shadow-[0_6px_16px_rgba(49,84,196,0.22)] active:translate-y-0">
          Get a Quote
        </button>
      </motion.div>
      ...
      <AnimatePresence>
        {activeDropdown && (
          <motion.div variants={dropdownVariants} initial="hidden" animate="visible" exit="exit" onMouseEnter={() => setHoveredNavItem(activeDropdown)} onMouseLeave={() => { setActiveDropdown(null); setHoveredNavItem(null); }} className="absolute left-3 right-3 top-[calc(100%-2px)] z-[90] hidden lg:block">
            <div className="mx-auto max-w-[980px] overflow-hidden rounded-b-2xl rounded-t-xl border border-white/[0.14] bg-[#080F29]/95 shadow-[0_18px_42px_rgba(0,0,0,0.45),0_0_30px_rgba(0,174,239,0.08)] backdrop-blur-xl">
              <div className="mx-auto h-px w-28 bg-gradient-to-r from-transparent via-[#00AEEF]/55 to-transparent" />

              <div className={`grid gap-5 px-5 py-5 xl:px-6 ${DROPDOWN_CONTENT[activeDropdown]?.length === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
                {DROPDOWN_CONTENT[activeDropdown]?.map((column, columnIndex) => {
                  const SectionIcon = column.sectionIcon;

                  return (
                    <div key={column.heading} className={`min-w-0 ${columnIndex > 0 ? "border-l border-white/[0.07] pl-5" : ""}`}>
                      <p className="mb-3 flex items-center gap-2 font-['Inter'] text-[9px] font-bold uppercase tracking-[0.16em] text-[#20C997]">
                        <SectionIcon size={13} strokeWidth={1.8} className="text-[#00AEEF]" />
                        {column.heading}
                      </p>

                      <div className="space-y-2">
                        {column.items.map((item) => (
                          <MegaMenuItem key={item.label} label={item.label} icon={item.icon} onClick={() => handleDropdownItemClick(item.label)} />
                        ))}
                      </div>
                    </div>
                  );
