  ...
  return (
    <>
      <motion.div initial={false} animate={{ opacity: docked ? 1 : 0 }} className="relative z-30 ml-auto flex items-center gap-2 lg:hidden">
        <button type="button" aria-label="Open search" onClick={() => setMobileSearchOpen(!mobileSearchOpen)} className={`flex h-9 w-9 items-center justify-center rounded-full border transition-all duration-300 sm:h-10 sm:w-10 ${mobileSearchOpen ? "border-[#00AEEF] bg-[#00AEEF]/10 text-[#00AEEF]" : "border-white/25 bg-white/[0.03] text-white"}`}>
      ...
      <MenuOverlay open={mobileMenuOpen} onClose={closeMenu} handleDropdownItemClick={handleDropdownItemClick} handleNavItemClick={handleNavItemClick} />
    </>
  );
};
